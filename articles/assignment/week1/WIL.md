...
+ id로 데이터 찾기
1. DTO 없이 전달하는 방법  
    + 컨트롤러 - > 서비스 : id라는 String 전달  
    + 서비스 -> DB : id값을 가진 엔티티 탐색

+ 만약 조건이 늘어난다면?  
    + 컨트롤러 -> 서비스 : id가 조건에 맞는지 확인하는 코드 추가  
    + 이후 id 라는 String 전달
      
+ 문제점 : Query구문 증가, if문 복잡도 증가  
      비즈니스 로직보다 컨트롤러 코드가 더 많아짐  
    
그래서 **NestJS**에서는  Class-validator의 데코레이터들을 DTO에 작성후 유효성 검사 

2. DTO 있이 전달하는 법
    + 컨트롤러 -> 서비스 : 찾는 정보와 제약조건을 RequestDTO에 넣어서 탐색
    + 서비스 -> DB : RequestDTO로 탐색 후 ResponseDTO로 결과 받음


## DTO와 Entity의 분리 - 요청(Request) DTO
요청시 DTO를 만들지 않고 기존 존재하던 Entity를 사용하면 불필요한 파일들 만들지 않아도 되고 더 간편하지 않을까 ?
요청에 맞게 Entity 자체에 제약조건을 걸어도 되지 않을까?

> 당연히 안된다

문제점  
1.Entity를 제약조건이 바뀔 때 마다 매번 수정 할 수 없다  
2.결과를 Entity자체로 반환하기 때문에 필요없는 정보가 출력된다.

## DTO와 Entity의 분리 - 응답(Response)DTO
결과도 DTO로 전송해야될까 ?  
엔티티로 결과를 보내면 생기는 문제는 다음과 같다

1. 불필요한 데이터가 포함 돼 있다-> join걸리면 데이터가 커지고 속도 저하 발생
2. 중요한 정보를 가리기 위한 후처리 작업이 더 오래 걸린다

> 요청 & 응답시에 DTO를 사용해라

정리하자면
1. DTO는 엔티티 내부 구현을 **캡슐화** 한다 
2. DTO는 전달해야 할 데이터 크기 조절이 가능하다
3. DTO는 필요한 데이터만 전달할수 있다.
