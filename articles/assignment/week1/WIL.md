...
- DTO와 Entity의 분리 - **요청**(Request) DTO
    - DTO를 사용하면 코드량을 줄이고 읽기 쉬워짐
    
    ---
    
    Q. DTO를 만들지 않고 기존 Entity를 사용하면 불필요한 파일 안만들어도 되니까 편하지 않을까?
    
    A. 기존에 데이터를 받을 것보다 훨씬 큰 데이터를 받아야 해서 통신에 쓸모없는 오버헤드가 일어날 수 있음
    A. 데이터 베이스에 수정사항이 생기면 엔티티를 사용한 모든 API에 영향이 가게됨
...
- DTO와 Entity의 분리 - **응답**(Response) DTO
    ...
    Q. 응답할때는 굳이 안만들어도 될 것 같은데…
    
    A. 프론트엔드에서 쓸모없는 값들을 받게됨!!(응답의 크기가 불필요하게 커짐)
    
    A. 비밀번호등 민감사항을 저장 → 가려서 보내려해도 유지보수가 어려워짐
    A. 설계가 노출이 됨 → 보안사고의 위험을 가지게됨 (보안이 취약해짐)